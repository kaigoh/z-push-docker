#!/bin/sh
sed -e "s/define('BACKEND_PROVIDER', '')/define('BACKEND_PROVIDER', 'BackendCombined')/" \
...
sed -e "s/define('IMAP_SERVER', 'localhost')/define('IMAP_SERVER', '"$MAILSERVER_ADDRESS"')/" \
    ...
    -e "s|imap_smtp_params = array()|imap_smtp_params = array('host' => '"$MAILSERVER_PROTOCOL$SMTPSERVER_ADDRESS"', 'port' => '"$SMTPSERVER_PORT"', 'auth' => true, 'username' => 'imap_username', 'password' => 'imap_password', 'verify_peer_name' => false, 'verify_peer' => false, 'allow_self_signed' => true)|" \
