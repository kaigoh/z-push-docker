...
define('IMAP_SERVER', ($_ENV['IMAP_SERVER'] ? $_ENV['IMAP_SERVER'] : null));

// connecting to default port (143)
define('IMAP_PORT', ($_ENV['IMAP_PORT'] ? $_ENV['IMAP_PORT'] : null));

// best cross-platform compatibility (see http://php.net/imap_open for options)
define('IMAP_OPTIONS', ($_ENV['IMAP_OPTIONS'] ? $_ENV['IMAP_OPTIONS'] : null));
...
define('IMAP_FOLDER_PREFIX', ($_ENV['IMAP_FOLDER_PREFIX'] ? $_ENV['IMAP_FOLDER_PREFIX'] : null));

// Inbox will have the preffix preppend (3 & 4 to true)
define('IMAP_FOLDER_PREFIX_IN_INBOX', false);

// Inbox folder name (case doesn't matter) - (empty in 4)
define('IMAP_FOLDER_INBOX', ($_ENV['IMAP_FOLDER_INBOX'] ? $_ENV['IMAP_FOLDER_INBOX'] : null));

// Sent folder name (case doesn't matter)
define('IMAP_FOLDER_SENT', ($_ENV['IMAP_FOLDER_SENT'] ? $_ENV['IMAP_FOLDER_SENT'] : null));

// Draft folder name (case doesn't matter)
define('IMAP_FOLDER_DRAFT', ($_ENV['IMAP_FOLDER_DRAFT'] ? $_ENV['IMAP_FOLDER_DRAFT'] : null));

// Trash folder name (case doesn't matter)
define('IMAP_FOLDER_TRASH', ($_ENV['IMAP_FOLDER_TRASH'] ? $_ENV['IMAP_FOLDER_TRASH'] : null));

// Spam folder name (case doesn't matter). Only showed as special by iOS devices
define('IMAP_FOLDER_SPAM', ($_ENV['IMAP_FOLDER_SPAM'] ? $_ENV['IMAP_FOLDER_SPAM'] : null));

// Archive folder name (case doesn't matter). Only showed as special by iOS devices
define('IMAP_FOLDER_ARCHIVE', ($_ENV['IMAP_FOLDER_ARCHIVE'] ? $_ENV['IMAP_FOLDER_ARCHIVE'] : null));
...
$imap_smtp_params = array(
        'host' => ($_ENV['SMTP_SERVER'] ? $_ENV['SMTP_SERVER'] : null),
        'port' => ($_ENV['SMTP_PORT'] ? $_ENV['SMTP_PORT'] : null),
        'auth' => true,
        'username' => ($_ENV['SMTP_USERNAME'] ? $_ENV['SMTP_USERNAME'] : null),
        'password' => ($_ENV['SMTP_PASSWORD'] ? $_ENV['SMTP_PASSWORD'] : null),
